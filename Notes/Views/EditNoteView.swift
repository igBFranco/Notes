    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing){
                Button("Salvar"){
                    DataController().editNote(note: note, title: title, content: content, date: date, imageData: selectedImage?.jpegData(compressionQuality: 0.8), latitude: latitude, longitude: longitude, context: managedObjContext)
                    dismiss()
                }
            }
        }
